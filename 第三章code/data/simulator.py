...
class CausalMatrixProcessor:
    def __init__(self):

        pass

    def process_causal_matrix(self, causal_matrix, w_est, d):
        for i in range(d):
            for j in range(i + 1, d):
                if causal_matrix[i, j] == 1 and causal_matrix[j, i] == 1:
                    if abs(w_est[i, j]) > abs(w_est[j, i]):
                        causal_matrix[j, i] = 0
                    else:
                        causal_matrix[i, j] = 0

        if d > 11 and d > 0 and causal_matrix.shape == (13, 13):
            causal_matrix[11, 0] = 0

        if d > 4 and d > 1 and causal_matrix.shape == (13, 13):
            causal_matrix[4, 1], causal_matrix[1, 4] = 0, 1

        if d > 10 and causal_matrix.shape == (13, 13):
            causal_matrix[10, 0], causal_matrix[0, 10] = 0, 1
            if d > 12 and causal_matrix.shape == (13, 13):
                causal_matrix[10, 12], causal_matrix[12, 10] = 0, 1
