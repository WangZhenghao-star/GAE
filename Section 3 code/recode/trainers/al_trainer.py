...
class ALTrainer(object):
    ...
    def __init__(self, n, d, model, lr, init_iter, penalty_lambda, alpha, beta, rho, rho_thresh,
                 h_thresh, l1_penalty, gamma, early_stopping,
                 early_stopping_thresh, seed, device=None, lambda_penalty=0.01):
        self.n = n
        self.d = d
        self.model = model
        self.lr = lr
        ...
        self.seed = seed
        self.device = device
        self.lambda_penalty = lambda_penalty  # Add the penalty term coefficient parameter
    ...
    def train_step(self, x, update_freq, alpha, rho):
        ...
        for _ in range(update_freq):
            self.lambda_penalty = self.penalty_lambda
            torch.manual_seed(self.seed)
            curr_mse, w_adj = self.model(x)
            curr_h = compute_h(w_adj)
            bidirectional_penalty = compute_bidirectional_penalty(w_adj, self.lambda_penalty)
...
def compute_bidirectional_penalty(w_adj, lambda_penalty):
    """
    Calculate the bidirectional edge penalty term.
    """
    # Calculate the bidirectional edge penalty term
    bidirectional_penalty = torch.sum(torch.abs(w_adj * w_adj.t()))
    bidirectional_penalty *= lambda_penalty
    return bidirectional_penalty