...
import torch
# Import the output_adj function from recode.utils to handle adjacency matrix output
from recode.utils import output_adj
# Import the GAE (Graph Autoencoder) class from recode.gae for causal discovery tasks
from recode.gae import GAE
# Import pandas library for data manipulation and analysis
import pandas as pd
# Import numpy library for numerical computations
import numpy as np
# Import the adj_cs function from recode.causal_strength to calculate causal strength
...
# Check if the 'CLASS' column exists in the DataFrame; if present, remove it
if 'CLASS' in X.columns:
    X = X.drop(columns=['CLASS'])
else:
    # Print a message if the 'CLASS' column is not found in the DataFrame
    print("The column 'CLASS' does not exist.")

# Convert the pandas DataFrame to a NumPy array and cast to float32 for model compatibility
